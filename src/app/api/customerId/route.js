...
export async function GET(req) {
    try {
        ...
        if (user.customerId) {
            return new Response(JSON.stringify({ success: true, customerId: user.customerId }), {
                status: 200,
            });
        } else {
            return new Response(JSON.stringify({ success: false, message: 'Customer ID not found for this user' }), {
                status: 404,
            });
        }
    } catch (error) {
